...
build_ipk() {
    local PKG_NAME="$1" PKG_DESC="$2" DEPENDS="$3"
    local FILES_DIR="$(pwd)/packages/$PKG_NAME/files"
    local WORK="$BUILD_DIR/$PKG_NAME"
    local DATA_DIR="$WORK/data" CTRL_DIR="$WORK/control"
    echo ""
    echo "-> Building $PKG_NAME..."
    mkdir -p "$DATA_DIR" "$CTRL_DIR"

    if [ -d "$FILES_DIR" ]; then cp -r "$FILES_DIR/." "$DATA_DIR/"; fi
    ...
    if [ -d "$DATA_DIR/www/cgi-bin" ]; then
        find "$DATA_DIR/www/cgi-bin" -type f -exec chmod 0755 {} \;
    fi
