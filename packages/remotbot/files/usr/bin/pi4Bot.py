...
def get_vnstat_traffic() -> str:
    try:
        ...
        vnstat_ok = run_command("which vnstat 2>/dev/null")
        if not vnstat_ok or "Error" in vnstat_ok:
            try:
                rx_bytes = tx_bytes = 0
                with open("/proc/net/dev") as f:
                    for line in f:
                        if iface in line:
                            parts = line.split()
                            rx_bytes = int(parts[1]); tx_bytes = int(parts[9]); break
                ...
                out += f"Error baca /proc/net/dev: {e}"
            return out
        ...
        try:
            raw = run_command(f"vnstat --json t -i {iface} 2>/dev/null")
            if raw and "Error" not in raw:
                data = json.loads(raw)
                if data and "interfaces" in data:
                    tops = data["interfaces"][0]["traffic"].get("top",[])[:5]
                    if tops:
                        out += "🏆 <b>Top 5 Hari Tertinggi:</b>\n"
                        for i, day in enumerate(tops, 1):
                            d = day.get("date",{})
                            date = f"{d.get('year','?')}-{d.get('month',0):02d}-{d.get('day',0):02d}"
                            out += f"{i}. <code>{date}</code>: {format_bytes(day.get('rx',0)+day.get('tx',0))}\n"
        except Exception: pass
        return out.strip()
    except Exception as e:
        return f"Error get_vnstat_traffic: {str(e)}"
...
def get_disk_info() -> str:
    try:
        out = "💿 <b>Disk Usage</b>\n\n"; df = run_command("df -h"); found = False
...
def get_services_keyboard(cfg: dict = None) -> InlineKeyboardMarkup:
    if cfg is None: cfg = load_config()
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(t("service_ctrl",cfg), callback_data="service_control")],
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cb.startswith("toggle_notify"):
        key = cb.replace("toggle_",""); new_val = "0" if cfg.get(key,True) else "1"
        save_config_uci(key,new_val); cfg2 = load_config()
        await query.edit_message_text(get_settings_text(cfg2), parse_mode='HTML', reply_markup=get_settings_keyboard(cfg2)); return
    if cb == "block_menu":
    ...
    if cb == "show_whitelist":
        ...
        text = ("📋 <b>MAC Whitelist:</b>\n\n" + "\n".join(f"✅ <code>{m}</code>" for m in wl)
                if wl else t("whitelist_empty",cfg))
